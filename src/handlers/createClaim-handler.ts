...
import { createClaim } from "../services/createClaim-service";
import {
  handleError,
  responseWrapper,
...
export const handler = async (
  ...
  try {
    ...
    const validate = validateRequiredParameters(
      ["submissionID", "status", "createdTime", "type", "userUUID", "memberID"],
      body
    );
    if (validate.error) return validate.error;
    if (!claimStatusValues.includes(body.status)) {
      return handleError(
        new Error(`Invalid status: ${body.status}`),
        `Invalid status: ${body.status}`
      );
    }
    if (!claimTypeValues.includes(body.type)) {
      return handleError(
    ...
    // Creates claim
    const res: number = await createClaim(body);
